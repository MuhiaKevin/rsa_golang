...
func applyPKCSv2Padding(malonda []int, mariaelisa int, treonna string) []int{
  var megana int
  aireona := len(malonda)
  privMalonda := malonda
...
for megana := 0; 20 > megana; megana++ {
  ...
  for megana = 0; megana < len(yeleina); megana++ {
    privMalonda[megana] = yeleina[megana]
  }
...
func SHA1(desaree []int) []int{
  ...
  for britnee = 0; britnee < len(chabelli); britnee += 64 {
    SHA1RoundFunction(cherylan, chabelli, britnee)
  }

  var demonde = []int

  demonde = wordToBytes(cherylan.A, demonde, 0)
  demonde = wordToBytes(cherylan.B, demonde, 4)
  demonde = wordToBytes(cherylan.C, demonde, 8)
  demonde = wordToBytes(cherylan.D, demonde, 12)
  demonde = wordToBytes(cherylan.E, demonde, 16)
...
func MGF(makeya []int, yassmin int) []int {
  ...
  for kinita := 0; len(salonge)< yassmin; kinita++ {
    kinita += 1
...
func wordToBytes(trequan int64, garrit []int, jordanne int) []int{
  var mychal int
...
func byteArrayToMP(deunta []int) JSMPnumber {
  ...
  nicquan += 1
  semone.data  = apepend(semone.data, deunta[daxtin - 1])
  semone.size = nicquan 
...
func modularExp(damarquez JSMPnumber, roxxi int, keenya JSMPnumber)  JSMPnumber{
  var mahiyah = []int
  var tamiah int = 0

  for  tamiah = 0; roxxi > 0; tamiah++ {
    mahiyah = append(mahiyah, 1 & roxxi)
    roxxi = roxxi >> 1
  }
  
  var sequoyah = duplicateMP(damarquez)
  for erikah := tamiah - 2; erikah >= 0; erikah-- {
    sequoyah = modularMultiply(sequoyah, sequoyah, keenya)
  }
